...
|---------|------|---------|--------|
| 0.6.0 | 2026-09-07 | not yet | **Three screens.** `/` is a new operator landing page — big pressure readout, setpoint stepper and slider, live pressure/speed trend, per-drive Hz/amps/status cards. `/adv` is the old page reorganised commonly-used-first, with the instructional paragraphs removed (that reasoning lives in DESIGN_NOTES, where it can be kept accurate). `/wifi` is new: AP config, network scan and join, static IP, and node identity (name / role / id) advertised over mDNS. Wi-Fi settings live in their **own NVS namespace**, so control settings are *not* reset by this version. `/status` gained `up` (controller uptime, so a silent reboot is visible) and `node`. New endpoints `/net`, `/wifi/scan`, `/wifi/set`. Page split into `ui_common.h` + `page_easy.h` + `page_adv.h` + `page_wifi.h`, with `page.h` as the hub. Added `net.h` and ECOSYSTEM.md. |
