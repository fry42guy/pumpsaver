...
Built so far: identity, AP + station, static addressing and mDNS in `net.h`,
the Network screen (`page_net.h`), and the MQTT publisher. Everything about a
*second node* is still plan. This is the plan the built parts were shaped to fit.
...
**MQTT** is built (`net.h`, publishing from a core-0 task) and is aimed at
ThingsBoard. It is deliberately **telemetry only, one way** — the node publishes
what it is doing and subscribes to nothing that can change what it does. That
keeps rule 1 intact: pull the broker's plug and every pump holds pressure
exactly as before. The day a dashboard needs to *write* a setpoint is the day
rule 4 has to be answered first — the local UI must show that the setpoint is
being written from elsewhere, or an operator will fight a control loop they
cannot see.
...
| E4 | Is there an existing MQTT broker or SCADA on any of these sites, or is ThingsBoard the whole answer? | gateway scope |
| ~~E5~~ | ~~81 % of the app partition is used at `v0.6.0`.~~ | **closed in `v0.10.0`** |

E5 is closed. The sketch was on a scheme offering 1.31 MB to the application and
was using 81 % of it; `v0.10.0` moved it to `app3M_fat9M_16MB` — 3 MB of
application (now ~38 % used) and a 9 MB FATFS partition for calibration logs.
It was cheap to change with one board on the bench and would have been
expensive with five in the field, which is the only reason it was done then.

The scheme also settles OTA, at no cost. `app3M_fat9M_16MB.csv` is:

```
app0      app   ota_0   0x10000   0x300000    3 MB
app1      app   ota_1   0x310000  0x300000    3 MB   <- the OTA target
ffat      data  fat     0x610000  0x9E0000    9.9 MB
```

**Two full 3 MB app slots already exist**, and `otadata` with them. OTA needs no
partition change and no sacrifice of application space — only the update code
and a way to serve the image. Pushing firmware to five wet plant rooms over USB
is the kind of thing that only has to happen once, so this is a cheap win
